...
def test_pdf_dependency_warning_when_parser_missing():
    mod = _load_module("ai_extraction_pdf_test", "premafirm_ai_engine/services/ai_extraction_service.py")
    ...
    try:
        ...
        text_out = svc._extract_attachment_text(DummyAttach())
    finally:
        builtins.__import__ = real_import

    assert text_out == ""
    assert any("PDF parser dependency missing" in w for w in svc._runtime_warnings)


def test_run_update_uses_driver_partner_directly():
    map_mod = _load_module("premafirm_ai_engine.services.mapbox_service", "premafirm_ai_engine/services/mapbox_service.py")
    sys.modules["premafirm_ai_engine.services.mapbox_service"] = map_mod
    run_mod = _load_module("premafirm_ai_engine.services.run_planner_service", "premafirm_ai_engine/services/run_planner_service.py")
    ...
    class CalendarModel:
        def create(self, vals):
            created.update(vals)
            return SimpleNamespace(write=lambda *_a, **_k: None)

    planner = run_mod.RunPlannerService(env={"calendar.event": CalendarModel()})
    run = SimpleNamespace(
        name="Run 1",
        vehicle_id=SimpleNamespace(driver_id=SimpleNamespace(id=42)),
        calendar_event_id=None,
        estimated_revenue=0.0,
        estimated_cost=0.0,
        id=5,
    )
    run.write = lambda vals: [setattr(run, k, v) for k, v in vals.items()]

    planner._update_run(run, {"total_drive_hours": 1.0, "total_distance_km": 10.0, "empty_distance_km": 2.0, "loaded_distance_km": 8.0})
    assert created["partner_ids"] == [(6, 0, [42])]
