...
Scope — the dangerous actions only:
  ...
  3. Portal-style confirms (ctx send_email=True — customer's own approval) send
     once, record the marker, and later staff attempts are refused.
...
class TestQuoteConfirmSafety(TransactionCase):
    ...
    def test_customer_approval_confirms_and_mails_once(self):
        """Customer's own approval (send_email ctx) still mails once — no more, no less."""
        order = self._make_order()
        order.with_context(send_email=True).action_confirm()
        self.assertEqual(order.state, 'sale')
        self.assertTrue(order.x_confirmation_email_sent_at)
        self.assertEqual(len(self._mails(order)), 1)
        # Later staff click on the same order: refused, no duplicate.
        with self.assertRaises(UserError):
            order.action_send_order_confirmation()
        self.assertEqual(len(self._mails(order)), 1)
