...
def test_load_section_parser_extracts_weight_from_table_style_labels():
    mod = _load_module("ai_extraction_weight_table_test", "premafirm_ai_engine/services/ai_extraction_service.py")
    svc = mod.AIExtractionService(env=None)
    raw_text = """
LOAD #1
Pallets
...
9,115
Pickup Address
55 Commerce Park Dr, Barrie, ON L4N 8Y1, Canada
Delivery Address
6350 Tomken Rd, Mississauga, ON L5T 2Y8, Canada
Delivery Date
Same Day
"""
    parsed = svc._parse_load_sections(raw_text)
    assert len(parsed["stops"]) == 2
    assert parsed["stops"][0]["weight_lbs"] == 9115.0
    assert not parsed["errors"]


def test_load_section_parser_does_not_parse_delivery_address_as_delivery_date():
    mod = _load_module("ai_extraction_delivery_date_label_test", "premafirm_ai_engine/services/ai_extraction_service.py")
    svc = mod.AIExtractionService(env=None)
    raw_text = """
LOAD #1
Pickup Address: 55 Commerce Park Dr, Barrie, ON
...
"""
    parsed = svc._parse_load_sections(raw_text)
    assert len(parsed["stops"]) == 2
    assert not [w for w in parsed["warnings"] if "could not parse delivery date" in w]
